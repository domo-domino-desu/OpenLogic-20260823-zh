...
\begin{align*}
  ...
  \intertext{$\fn{Succ}$ 是一个以数字~$a$ 为自变元的函数，它求值为另一个函数，即 $\lambd[fx][f ({a}
      f x)]$。该函数本身并不是Church数。然而，如果自变元 $a$ 是Church数，它就归约为一个Church数。考虑：}
...
\intertext{定义。第一个加法项的工作方式如下：$\fn{Add}$ 首先接受两个数字 ${a}$ 和 ${b}$ 作为自变元。结果是一个接受 $f$ 和 $x$ 并返回 $af(bfx)$ 的函数。如果 $a$ 和 $b$ 是Church数 $\num{n}$ 和 $\num{m}$，那么这归约为 $f^{n+m}(x)$，它与 $f^{n}(f^{m}(x))$ 等同。或者，一步步来看：}
  (\lambd[{a}{b}][\lambd[fx][{a} f ({b} f x)]])\num n\,\num m & \redone
  \lambd[fx][\num{n}\, f (\num {m}\, f x)] \\
  & \redone \lambd[fx][\num{n}\, f (f^m x)] \\
  & \redone \lambd[fx][f^n (f^m x)] \ident \num {n+m}.
\intertext{第二种加法表示 $\fn{Add'}$ 的工作方式不同：应用到两个Church数 $\num{n}$ 和 $\num{m}$ 上时，}
\fn{Add}' \num n \,\num m
& \redone \num{n}\, \fn{Succ}\, \num{m}.
...
\begin{proof}
为了理解其工作原理，假设我们将 $\fn{Mult}$ 应用到Church数 $\num{n}$ 和 $\num{m}$ 上：$\fn{Mult} \, \num{n} \, \num{m}$ 归约为
  $\lambd[fx][\num{n}(\num{m}\, f)x]$。项 $\num{m} f$ 定义了一个将 $f$ 对其自变元应用 $m$ 次的函数。因此，$\num{n} (\num{m} f) x$ 将“将 $f$ 应用 $m$ 次”这个函数本身对 $x$ 应用 $n$ 次。换句话说，我们对 $x$ 应用 $f$ 共 $n\cdot m$ 次。但所得的范式正是Church数 $\num{nm}$。
\end{proof}
...
在Church数上定义前驱和减法并不像我们想象的那么简单：它需要用到序对的编码。

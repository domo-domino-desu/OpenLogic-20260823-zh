...
$\lambd$-抽象 $\lambd[x][M]$ 表示单自变元函数，若要定义接受多个自变元的函数，这无疑是个不小的限制。一种解决办法是扩展 $\lambd$-演算，允许构成有序对、三元组等；这样一来，例如，三元函数 $\lambd[x][M]$ 便会要求其自变元是一个三元组。然而，通过\emph{Curry化}来实现这一点更为方便。
...
x+y$。通过这种方式，单自变元函数 $g$ 就能起到与二元加法函数相同的作用。“Curry化”指的就是这种将二元函数转化为单自变元函数（其值本身也是单自变元函数）的技巧。

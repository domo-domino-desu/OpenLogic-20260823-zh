...
Schröder-Bernstein（Schröder--Bernstein）定理证明的一些背景，与我们在
\olref[his][set][pathology]{sec} 节中讨论过的历史有关联。
回想一下，1877年，Cantor 证明了正方形上的点与其一条边上的点恰好一样多。
在此，我们将给出他第一次尝试的证明。

令 $\unitline$ 为单位线段，即点集 $[0,1]$。
令 $\unitsquare$ 为单位正方形，即点集 $\unitline
\times \unitline$。
用这些记号来说，Cantor 证明了 $\cardeq{\unitline}{\unitsquare}$。
他给 Dedekind写了一张便条，实质上包含了如下论证。
...
遗憾的是，正如 Dedekind 向 Cantor 所指出的，这并未解决最初的问题。
考虑 $0.\dot{1}\dot{0} =
...
总而言之：Dedekind 指出，由于存在某些循环小数展开的可能，Cantor 的函数 $f$ 是一个单射但\emph{不是}满射。所以 Cantor 只证明了 $\cardle{\unitsquare}{\unitline}$，而\emph{没有}证明 $\cardeq{\unitsquare}{\unitline}$。

Cantor 几乎立刻回信给 Dedekind，实质上建议可以按如下方式完成证明：
...
然而，Cantor 当时无法用这些术语写下最后一行，因为 Schr\"{o}der--Bernstein 定理尚未得到证明。
事实上，尽管 Cantor 后来将此表述为一个一般性猜想，但直到1897年它才得到令人满意的证明。（因此，在1877年晚些时候，Cantor 给出了 \olref{thm:cantorplane} 的另一个证明，该证明并未借助 Schr\"{o}der--Bernstein 定理。）

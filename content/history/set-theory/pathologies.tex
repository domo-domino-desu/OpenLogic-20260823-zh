...
19世纪30年代左右，Bolzano发现了一个
\emph{处处连续}，但\emph{无处可导}的函数。
（遗憾的是，Bolzano 从未发表这一发现；数学家们直到1872年才首次接触到这个想法，这要归功于 Weierstrass对同一想法的独立发现。）\footnote{这段历史记载在维基百科关于
\href{http://en.wikipedia.org/wiki/Weierstrass_function}{Weierstrass函数}的文章的极其详尽的脚注中。}退一步说，这也相当令人惊讶。
...
进一步的“病态”接踵而至。1874年1月5日，Cantor 写信给 Dedekind，提出了这样一个问题：
...
受 Cantor 结果的启发，Peano开始思考是否可能将一条线\emph{光滑地}映射到一个平面上。这将是一条\emph{填满空间的曲线}。在 \citeyear{Peano1890}，Peano 恰恰构造出了这样一条曲线。这确实是反直觉的：Euclid曾将线定义为“没有宽度的长度”（第 I 卷，定义 2），但 Peano 却表明，通过适当地卷曲一条线，其长度可以转化为宽度。
在 \citeyear{Hilbert1891}，Hilbert描述了一条稍具直观性的空间填充曲线，并附带了一些图示。该曲线是逐次构造的，以下是其构造的前六个阶段：

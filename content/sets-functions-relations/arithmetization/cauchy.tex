...
\olsection{附录：实数作为Cauchy序列}

在\olref[cuts]{sec}中，我们用Dedekind分割构造了实数。
本节将解释另一种构造方法。它基于 Cauchy 对（即我们现在所称的）Cauchy 序列的定义；
但用这一定义来\emph{构造}实数则要归功于其他十九世纪的学者，特别是 Weierstrass（魏尔斯特拉斯）、Heine（海涅）、M\'{e}ray（梅雷）和 Cantor 。（一段精彩的历史介绍，可参阅 \citeauthor{OConnorRobertson:RN} \citeyear{OConnorRobertson:RN}。）
...
\begin{defn}\ollabel{def:CauchySequence}
  一个函数 $f: \Nat \to \Rat$ 是\emph{Cauchy 序列}，当且仅当对于任意正有理数 $\epsilon \in \Rat$，都有 $(\exists \ell \in
...
那么，一个自然的想法是，实数就\emph{是}某个Cauchy 序列。但是，正如构造 $\Int$ 和 $\Rat$ 时那样，这会过于天真：对于任意给定的实数，都有多个不同的Cauchy 序列指示该实数。一个简单的方法可以说明这一点。给定一个Cauchy 序列~$f$，定义 $g$ 为一个除 $g(0)\neq
f(0)$ 外与~$f$ 完全相同的函数。由于这两个序列在第一个数之后处处相同，我们（最终）会想说它们有相同的极限（就 \olref{def:CauchySequence} 中采用的极限意义而言），因此应视为“定义”了同一个实数。所以，我们确实应该把这些Cauchy 序列看作同一个实数。

因此，我们再次需要在Cauchy 序列上定义一个等价关系，并将实数等同于等价类。首先我们需要函数趋于 $0$ 的概念。对于任意函数 $h : \Nat \to \Rat$，称\emph{$h$ 趋于 $0$}当且仅当对任意正的有理数 $\epsilon \in \Rat$，我们有 $(\exists \ell \in \Nat)(\forall n > \ell)|h(n)| <
...
$\Rat$ 的Cauchy 序列在 $\Realequiv$ 下的等价类。

\begin{prob}
设对于每个 $n$，$f(n) = 0$。令 $g(n) = \frac{1}{(n+1)^2}$。证明
两者都是Cauchy 序列，并且实际上两个函数的极限都是 $0$，从而也有 $f \sim_{\Real} g$。
\end{prob}
...
g(n)$。当然，我们还需要检查当 $f$ 和 $g$ 是Cauchy 序列时， $(f + g)$、
$(f-g)$ 和 $(f\times g)$ 中的每一个也都是Cauchy 序列；确实如此，我们将这留给你来完成。
...
%\begin{align*}
%	\equivrep{f}{} \leq \equivrep{g}{} \text{ 当且仅当 }&\equivrep{f}{} = \equivrep{g}{} \text{ 或 }\\
...
\begin{thm}
每个有上界的非空Cauchy 序列集都有最小上界。
\end{thm}

\begin{proof}[证明思路]
设 $S$ 为任意有上界的非空Cauchy 序列集。那么存在某个 $p \in \Rat$，使得 $p_{\Real}$ 是 $S$ 的一个上界。令 $r \in S$；则存在某个 $q \in \Rat$ 使得 $q_{\Real} < r$。因此，如果 $S$ 存在最小上界，那么它介于 $q_{\Real}$ 和 $p_{\Real}$ 之间（含端点）。
...
\begin{align*}
	...
	\end{cases}
\end{align*}
$f$ 和 $g$ 都是Cauchy 序列。（这可以很容易地验证，但我们留作练习。）注意函数 $(f-g)$ 趋于 $0$，因为 $f$ 和 $g$ 之间的差值每一步都减半。因此 $\equivrep{f}{} = \equivrep{g}{}$。

我们首先证明 $\equivrep{f}{}$ 是 $S$ 的一个上界，即 $(\forall h \in S)\equivrep{h}{} \leq \equivrep{f}{}$。（我们将在过程中引用 \olref{thm:cauchyorderedfield}。）令 $h \in S$ 并假设（为归谬）$\equivrep{f}{} < \equivrep{h}{}$，从而 $0_{\Real} < \equivrep{(h-f)}{}$。由于 $f$ 是一个单调递减的Cauchy 序列，存在某个 $n \in \Nat$ 使得 $\equivrep{(c_{f(n)} - f)}{} < \equivrep{(h-f)}{}$。所以：
\[
	(f(n))_{\Real} = \equivrep{c_{f(n)}}{} < \equivrep{f}{} + \equivrep{(h-f)}{} = \equivrep{h}{},
\]
这与以下事实矛盾：根据构造，$\equivrep{h}{} \leq (f(n))_{\Real}$。

接下来我们证明 $\equivrep{f}{} = \equivrep{g}{}$ 是 $S$ 的\emph{最小}上界。令 $j$ 为任意Cauchy 序列并假设 $\equivrep{j}{} < \equivrep{g}{}$。与上面的推理类似（利用 $g$ 是\emph{递增}的事实），存在 $n \in \Nat$ 使得 $\equivrep{j}{} < (g(n))_{\Real}$。但由构造，存在 $h \in S$ 使得 $(g(n))_{\Real} \leq \equivrep{h}{}$，所以 $\equivrep{j}{} < \equivrep{h}{}$，因此 $\equivrep{j}{}$ 不是 $S$ 的上界。
\end{proof}

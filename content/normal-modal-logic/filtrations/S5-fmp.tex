...
\begin{proof}
  由 \olref[frd][es5]{prop:S5=univ}，如果 $!A$ 在某个自反且Euclid的模型的某个世界上为真，那么它在某个全通模型的某个世界上也为真。
  由 \olref{prop:univ-fin}，它在某个有限全通模型（即对该模型经由 $!A$ 的子公式集进行过滤所得的模型）的某个世界上为真。
  每个全通模型也都自反且Euclid；因此 $!A$ 在某个有限的自反且Euclid的模型的某个世界上为真。
\end{proof}
...
\begin{prob}
  找出一个对称（或传递，或Euclid）模型的非对称（或非传递，或非Euclid）过滤。
\end{prob}

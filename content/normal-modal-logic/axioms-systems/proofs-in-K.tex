...
为了在最小的模态系统中练习证明，我们将展示
\olref[syn][sch]{tab:valid-invalidSchemas} 左侧的有效公式均可在 \Log{K} 中给出证明。
...
\begin{proof}
  ...
  \end{derivation}
  最后一行即 $\lnot \Box p \lif \Diamond\lnot p$，因为 $\Diamond$ 定义为 $\lnot\Box\lnot$。第 $5$ 行的公式是重言式
  \[
    (p \lif q) \lif (\lnot q \lif \lnot p).
  \]
  的一个实例。

...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item $\Log{KB5} \Proves \Ax{4}$:
      \begin{derivation}
        1. & $\Log{KB5} \Proves \Box!A \lif \Box\Diamond\Box !A$ & \Ax{B} 以
        $\Box!A$ 代 $p$ \\
...
\begin{defn}
  按照惯例，我们定义 \Log{S4} 为系统 \Log{KT4}，\Log{S5} 为系统 \Log{KTB4}。
\end{defn}

下列命题表明，经典系统 \Log{S5} 具有多种等价的公理化方式。这不足为奇，因为公理的各种组合刻画的都是等价关系（参见 \olref[frd][es5]{prop:equivalences}）。

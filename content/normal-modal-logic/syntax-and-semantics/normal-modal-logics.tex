...
我们将看到定义模态逻辑的两种主要方式。一种是证明论的方式，即将其定义为在特定推导系统中可推导出的公式集合。另一种是模型论的方式，即将其定义为在特定模型类中有效的公式集合。这与普通命题逻辑（乃至一阶逻辑）中的情形相对应。在那里，我们同样可以用两种不同的方式刻画一个公式集合，例如，既可以将其定义为在所有真值赋值下均为真的重言式集合，也可以将其定义为某个推导系统中所有可推导公式的集合。对于正规模态逻辑，利用关系模型和公理化（Hilbert型）证明系统，这种以双重方式刻画模态逻辑的做法是可行的。
\end{document}
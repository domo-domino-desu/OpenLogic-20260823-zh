...
\begin{proof}
  ...
  \item 我们采用逆否证法，假设框架 $\mModel{F} = \tuple{W, R}$ 不是欧几里得的，并证明它不满足~\Ax{5}（即 $\mModel{F} \Entails/ \Diamond p \lif
...
\Log{S5} 则是在所有自反、对称且欧几里得的框架中有效的公式集合，也就是在所有 \Ax{T}、\Ax{B} 和 \Ax{5} 都有效的框架中有效的公式集合。
...
\begin{prop}\ollabel{prop:relation-facts}
  ...
  \item 如果 $R$ 是对称的，那么它是传递的当且仅当它是欧几里得的。
  \item 如果 $R$ 是对称的或欧几里得的，那么它是弱定向的（具有“菱形性质”）。
  \item 如果 $R$ 是欧几里得的，那么它是弱连通的。

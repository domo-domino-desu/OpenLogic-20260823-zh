...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \Delta_2, \lnot !A)$。
    \item $\pi$ 以 $\RightR{\lif}$ 结束，主公式为~$!A \lif !B$。我们同样有两种情况，取决于 $!A \lif !B$ 属于第一部分还是第二部分。推导~$\pi$ 要么结束于
    \[
    \AxiomC{}
    \RightLabel{$\pi_1$}
    \Deduce$\maeh{!A, \Gamma_1}{\Gamma_2} \fCenter
    ...
    \cap \Lang L(\Gamma_2, \Delta_2)$。现在回想一下，我们假定了不存在!!{function}。
    这意味着项~$t$ 只能是!!a{constant}，即 $t \ident b$。如果 $b \notin \Lang L(!C_1)$
    ...
    \lexists[x][!A(x)]) \cap \Lang L(\Gamma_2, \Delta_2)$ 呢？首先，把~$!C_1$
    中 $b$ 的所有出现都替换为!!a{variable}~$y$，我们就可以写成 $!C_1 \ident \Subst{!C_1'}{b}{y}$，
    其中 $!C_1'$~不含~$b$。此外，要么 $b \notin

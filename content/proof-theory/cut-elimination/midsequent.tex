...
\emph{埃尔布朗 析取式}。由于 $!B$ 是
无量词的，这个析取式也是无量词的。因此，如果它是
可推导的，就可以通过无切割证明来推导，进而
通过一个只使用命题推理的推导来获得。故而，它
是一个重言式。

埃尔布朗 定理的困难部分在于证明：对每个可推导的
公式~$\lexists[x][!B(x)]$，都存在一个 埃尔布朗
析取式。其逆命题，即如果
$\lexists[x][!B(x)]$ 有一个 埃尔布朗 析取式，那么它就是可证的，
则很简单。事实上，埃尔布朗 析取式语义地后承 $\lexists[x][!B(x)]$。
...
为了从~$\Sequent
\lexists[x][!B(x)]$ 的一个推导~$\pi$ 中提取 埃尔布朗 析取式，考虑无切割推导~$\pi$ 中所有 $\RightR{\lexists}$
推理都在末尾的情况：
\[
\AxiomC{}
\RightLabel{$\pi_1$}

...
广义的\emph{停机问题}是指，给定可计算函数的描述~$e$（如程序）与数~$n$，判定该函数在输入~$n$ 上的计算是否\emph{停机}（即是否产生结果）。众所周知，Alan 图灵 证明了该问题本身无法由可计算函数解决，即函数
\[
h(e, n) =
\begin{cases}
...
在部分递归函数的语境下，程序的描述可由 Kleene（克林）范式定理中给出的指标~$e$ 充当。若 $f$ 是部分递归函数，则使得范式定理中方程成立的任意~$e$，都是~$f$ 的指标。给定数~$e$，克林 范式定理指出，\[
...
\end{enumerate}
若 $h$ 是部分递归的，则 $d$ 也将是部分递归的。因此，根据 克林 范式定理，$d$ 有一个指标~$e_d$。

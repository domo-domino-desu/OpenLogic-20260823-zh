...
$s^m_n(x, a_0, \dots, a_{m-1})$，该程序对应于由剩余自变元构成的 $n$ 元函数。\iftag{TMs}{若将 $e$ 视为一台Turing机的描述，那么 $s^m_n(e, a_0, \dots, a_{m-1})$ 便是这样一台Turing机：当输入为 $y_0$, \dots,~$y_{n-1}$ 时，它将 $a_0$, \dots,~$a_{m-1}$ 前置到输入串上，并运行~$e$。由此，每个 $s^m_n$ 不过是一个寻找相应Turing机编码的原始递归函数。}{}
\end{explain}

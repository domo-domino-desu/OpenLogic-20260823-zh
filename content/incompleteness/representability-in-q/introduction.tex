% Part: incompleteness
哥德尔 不完全性定理断言，对于某些足够强的形式系统，存在该语言中的一些语句，在系统内既不能被证明也不能被证否。这适用于包含了足够算术的系统，例如 鲁滨逊 算术或 皮亚诺 算术。不可判定语句并非只有单一的一个，在这些理论中存在着无穷多个这样的不可判定语句。粗略地说，该定理陈述如下：如果一个形式系统包含了足够算术，是一致的，并且是可公理化的，那么它是不完全的。
...
我们将描述一种满足此条件的极小理论，称为“$\Th{Q}$”（有时也称作“鲁滨逊 的 $Q$”，以 Raphael 鲁滨逊 命名）。
...
比 $\Th{Q}$ 更强的理论（称为 \emph{皮亚诺 算术} $\Th{PA}$）是通过向 $\Th{Q}$ 添加归纳模式得到的：
\[
...
事实上，要找到关于自然数的、在 皮亚诺 算术中\emph{不能}被证明的“自然”语句，是需要花相当大功夫的！

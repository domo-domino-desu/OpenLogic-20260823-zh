...
这并不完全精确，因为我们的过程处理的对象是推导和公式，而不仅仅是数字，并且我们还没有确切解释为什么“枚举所有可能的推导”是机械可行的。但正如我们所见，可以用Gödel数来编码项、公式和推导。我们也引入了一个精确的计算模型，即一般递归函数。而且我们已经看到，关系 $\Prf[\Th{Q}](d,y)$ 是（原始）递归的，该关系成立当且仅当 $d$ 是这样一个推导的Gödel数：该推导是从~$\Th{Q}$ 的公理出发对Gödel数为~$y$ 的公式所作的推导。我们需要的其他原始递归函数还有 $\fn{num}$（\olref[art][trm]{prop:num-primrec}）和 $\fn{Subst}$（\olref[art][sub]{prop:subst-primrec}）。利用这些函数，可以通过极小化来定义~$f$；因此 $f$ 是递归的。
...
\num{n_k}, (s)_1)$ 的一个推导的Gödel数。因此，寻找这样的~$s$ 就好比在非形式化证明中寻找数对 $d$ 和 $m$。而一个“寻找”这样的 $s$ 的可计算函数可以通过正则极小化来定义。注意 $R$ 是正则的：对每个 $n_0$, \dots, $n_k$，都存在 $\Th{Q} \Proves !A_f(\num{n_0}, \dots,

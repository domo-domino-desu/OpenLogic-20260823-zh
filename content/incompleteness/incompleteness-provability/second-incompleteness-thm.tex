...
希尔伯特纲领试图为数学提供一个一致的基础，并通过证明数学的形式系统是一致的来确保其可靠性。% Part: incompleteness 如果它成功了，那将是伟大的成就：它将意味着，我们拥有一个一致的数学系统，这个系统可以可靠地用于发现真理。% Chapter: incompleteness-provability Gödel（哥德尔）不完全性定理表明，Hilbert（希尔伯特）纲领无法实现：任何足够强的一致数学系统的一致性，都无法在该系统自身内部得到证明。更具体地说，Gödel 第一定理指出，对于任何足够强、一致且可公理化的理论~$T$，都存在该理论的语句~$G$，使得 $T$ 既不能证明~$G$，也不能证明~$G$ 的否定。这些定理对数学基础领域是一个冲击，并对数学哲学产生了深远影响。它们表明，对于 Hilbert 所希望的那种完备性，存在根本性的限制。% Section: second-incompleteness-thm
...
阅读 哥德尔 的论证概要很有教益，因为定理的得出就像绝妙的点睛之笔一样。论证过程如下。设 $!G_{\Th{PA}}$ 是我们在 \olref[1in]{thm:first-incompleteness} 的证明中构造的 哥德尔 语句。我们已经证明了“如果 $\Th{PA}$ 是一致的，那么 $\Th{PA}$ 不能推导 $!G_{\Th{PA}}$”。如果我们将此结论\emph{在} $\Th{PA}$ 中形式化，就得到了 \[
\OCon[\Th{PA}] \lif \lnot \OProv[\Th{PA}](\gn{!G_{\Th{PA}}}).
\] 的证明。现在假设 $\Th{PA}$ 推导 $\OCon[\Th{PA}]$。那么它就推导 $\lnot
\Prov[\Th{PA}](\gn{!G_{\Th{PA}}})$。但由于 $!G_{\Th{PA}}$ 是一个 哥德尔 语句，这就等价于 $!G_{\Th{PA}}$。所以 $\Th{PA}$ 推导 $!G_{\Th{PA}}$。

但是：我们知道如果 $\Th{PA}$ 是一致的，它就不能推导 $!G_{\Th{PA}}$!{} 因此，如果 $\Th{PA}$ 是一致的，它就不能推导 $\OCon[\Th{PA}]$。

为了使论证更精确，我们将令 $!G_{\Th{PA}}$ 是 $\Th{PA}$ 的 哥德尔 语句，并利用可证性条件 (P1)--(P3) 来证明 $\Th{PA}$ 推导 $\OCon[\Th{PA}] \lif
!G_{\Th{PA}}$。这将表明 $\Th{PA}$ 不能推导 $\OCon[\Th{PA}]$。以下是在~$\Th{PA}$ 中进行的证明概要。（为简单起见，我们省略 $\Th{PA}$ 的下标。）
\begin{align}
& !G \liff \lnot \OProv(\gn{!G}) \ollabel{G2-1}\\
& \qquad\text{$!G$ 是一个 哥德尔 语句}\notag \\
...
\begin{digress}
这里的启示是，没有任何“合理的”一致数学理论能够推导出其自身的一致性语句。假设 $\Th{T}$ 是一个包含 $\Th{Q}$ 和 希尔伯特 “有穷”推理（无论那可能是什么）的数学理论。那么，整个 $\Th{T}$ 无法推导 $\Th{T}$ 的一致性语句，因此，其有穷片段更是无法推导~$\Th{T}$ 的一致性语句。在这个意义上，不可能存在针对“全部数学”的有穷一致性证明。

在解释“有穷”这个术语时有一定的灵活性，哥德尔 在 1931 年的论文中也承认，可能存在某些我们认为是“有穷”的东西，但它们却超出了 希尔伯特 想要形式化的数学范围。但 哥德尔 的态度是宽容的；如今，很难想象我们会找到某种可以合理地称为“有穷”、却又无法在（比如说）带选择公理的 策梅洛--弗兰克尔 集合论 $\Th{ZFC}$ 中被形式化的东西。
\end{digress}

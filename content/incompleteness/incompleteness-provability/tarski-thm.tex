...
\begin{proof}
回忆 Kleene 范式定理，它断言每个部分可计算函数~$f$ 都有一个指标~$e$，使得 $f(x) =
...
那么 $\Th{TA}$ 本身呢？它在算术中是可定义的吗？也就是说，集合 $\Setabs{\Gn{!A}}{\Sat{N}{!A}}$ 在算术中是可定义的吗？Tarski（塔斯基）定理对此给出了否定的回答。
...
Tarski 将这一分析应用到关于真理的更一般的哲学概念上。对于任何语言 $L$，Tarski 认为，一个关于 $L$ 的充分的真理概念必须对每个语句 $X$ 满足
\begin{quote}
``$X$'' 是真的当且仅当 $X$。
\end{quote}
Tarski 常被引用的以英语为例的句子是：
\begin{quote}
``雪是白的''是真的当且仅当雪是白的。
\end{quote}
然而，对于任何强到足以表示对角函数的语言，以及该语言中的任何谓词 $T(x)$，我们都能构造出一个满足``$X$ 当且仅当并非 $T(\text{`$X$'})$''的语句 $X$。考虑到我们不会希望一个真理谓词判定某些语句既真又假，Tarski 得出结论：除非以某种方式超出该语言的界限，否则我们无法为该语言中的所有语句规定一个真理谓词。换言之，一个语言的真理谓词无法在该语言自身内被定义。

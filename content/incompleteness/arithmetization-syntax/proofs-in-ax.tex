...
并原始递归地表达其基本性质与关系。
有些操作很简单：例如，给定一个推导的哥德尔数~$d$，
$(d)_{\len{d}-1}$ 给出其末尾公式的哥德尔数。
有些则困难得多。我们至少会概述如何做到这一点。目标是证明关系
“$\delta$ 是从~$\Gamma$ 到~$!A$ 的推导”在 $\delta$ 和 $!A$ 的哥德尔数上是原始递归的。
\end{explain}
...
\begin{proof}
我们必须证明，公式与推导的哥德尔数之间的相应关系是原始递归的。
\begin{enumerate}
\item 我们有一个给定的公理模式列表，$!A$ 是一条公理，如果它符合其中某个模式的形式。
  ...
  我们能够对 $!A$ 的哥德尔数 $n$ 测试相应的性质，因为序列的拼接是原始递归的，
  且 $!B$ 和 $!C$ 的哥德尔数必定小于 $!A$ 的哥德尔数——
...
\item $\delta$ 中的一行由 \QR{} 证成，如果它具有形式 $!B \lif \lforall[x][!A(x)]$，
  ...
  \item $c$ 不在 $!B$ 中出现。
  \end{enumerate}
  所有这些都可以原始递归地测试，因为 $!B$、$!A(x)$ 和 $x$ 的哥德尔数小于第 $i$ 行公式的哥德尔数，
  而 $a$ 的哥德尔数小于第 $j$ 行公式的哥德尔数：
  \begin{multline*}
    ...
    \land \fn{Sent}(\fn{Subst}(a,c,x)) \land {} \bforall{k <
      \len{b}}{(b)_k \neq (c)_0})
  \end{multline*}
  这里我们假设 $c$ 和 $x$ 分别是作为项考虑的变元和常项的哥德尔数
  （即，不是它们的符号编码）。
...
\item $d$ 是一个正确推导的哥德尔数，当且仅当其中的每一行都是公理，或者由肯定前件或 \QR{} 证成。
  因此：
  \[
...
具有哥德尔数 $z$ 的语句是否具有这种形式，是可以原始递归地检验的。
因此，我们不把 $x$ 视为具有哥德尔数 $y$ 的语句\emph{从 $\Gamma$} 推导的哥德尔数，
而是把 $x$ 视为从 $\emptyset$ 推导出上述形式的一个嵌套条件句的哥德尔数。
...
\begin{align*}
  ...
  &\qquad\fn{Deriv}(x)).
\end{align*}
$s$ 的界限是这样得到的：每个 $(s)_i$ 都是该推导最后一行公式的某个子公式的哥德尔数，因而小于 $(x)_{\len{x}-1}$。

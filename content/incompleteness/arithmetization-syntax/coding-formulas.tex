...
\olsection{编码 \printtoken{P}{formula}}

一旦原始递归地定义了关系 $\fn{Term}(x)$，我们便可用它来原始递归地定义公式对应的关系 $\fn{Frm}(x)$。

\begin{prop}
关系 $\fn{Atom}(x)$（当且仅当 $x$ 是原子公式的哥德尔数时成立）是原始递归的。
\end{prop}

\begin{proof}
$x$ 是原子公式的哥德尔数，当且仅当以下条件之一成立：
\begin{enumerate}
...
关系 $\fn{Frm}(x)$（当且仅当 $x$ 是公式的哥德尔数时成立）是原始递归的。
\end{prop}
...
关系 $\fn{FreeOcc}(x, z, i)$（当且仅当哥德尔数为~$x$ 的公式的第~$i$ 个符号是哥德尔数为~$z$ 的变元的自由出现时成立）是原始递归的。
\end{prop}
...
\begin{prop}
性质 $\fn{Sent}(x)$（当且仅当~$x$ 是语句的哥德尔数时成立）是原始递归的。
\end{prop}

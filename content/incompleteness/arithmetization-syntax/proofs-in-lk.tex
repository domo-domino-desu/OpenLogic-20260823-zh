...
\olsection{$\Log{LK}$ 中的\usetoken{P}{derivation}}
...
若 $\Gamma \Sequent \Delta$ 是一个矢列，则 $\Gamma \Sequent \Delta$ 的一个哥德尔数是
\[
...
\begin{ex}
  ...
  \end{prooftree}
  初始矢列的哥德尔数为 $p_0 = \tuple{0,
    \Gn{!A \Sequent !A}}$。结束于 $\LeftR{\land}$ 结论的推导的哥德尔数为 $p_1 =
    \tuple{1, p_0, \Gn{!A \land !B \Sequent !A}, 9}$（$1$ 是因为 $\LeftR{\land}$ 有一个前提，加上结论 $!A \land !B \Sequent !A$ 的哥德尔数，而 $9$ 是 $\LeftR{\land}$ 的编码数字）。整个推导的哥德尔数则是 $\tuple{1, p_1, \Gn{\Sequent (!A \land !B) \lif !A)}, 14}$，即：
  \[
...
有些操作是简单的：例如，给定一个推导的哥德尔数 $p$，$\fn{EndSeq}(p) = (p)_{(p)_0+1}$ 给出其末矢列的哥德尔数，$\fn{LastRule}(p) = (p)_{(p)_0+2}$ 给出其最后规则的代码。
由 \[
  \len{s} = 2 \land \bforall{i<\len{(s)_0} + \len{(s)_1}}{\fn{Sent}(((s)_0 \concat (s)_1)_i)}
\] 定义的性质 $\fn{Sequent}(s)$ 对 $s$ 成立当且仅当 $s$ 是一个由语句组成的矢列的哥德尔数。
有些操作则要困难得多。我们至少会概述如何做到这一点。目标是证明关系“$\pi$ 是从 $\Gamma$ 到 $!A$ 的推导”是关于 $\pi$ 和 $!A$ 的哥德尔数的原始递归关系。
\end{explain}

\begin{prop}\ollabel{prop:followsby}
  性质 $\fn{Correct}(p)$（成立当且仅当哥德尔数为 $p$ 的推导 $\pi$ 中最后一个推理是正确的）是原始递归的。
\end{prop}

\begin{proof}
  $\Gamma \Sequent \Delta$ 是初始矢列，当且仅当要么存在语句~$!A$ 使得 $\Gamma \Sequent \Delta$ 是 $!A \Sequent !A$，要么存在项~$t$ 使得 $\Gamma \Sequent \Delta$ 是 $\emptyset \Sequent \eq[t][t]$。用哥德尔数表示，$\fn{InitSeq}(s)$ 成立当且仅当
  \begin{align*}
  ...
  我们还要证明，对于每个推理规则~$R$，关系 $\fn{FollowsBy}_R(p)$ 是原始递归的，其中 $\fn{FollowsBy}_R(p)$ 成立当且仅当 $p$ 是推导~$\pi$ 的哥德尔数，且 $\pi$ 的末矢列是通过对~$R$ 的正确运用从 $\pi$ 的直接子推导得到的。
  ...
  我们只需将这一点翻译成哥德尔数。如果 $s = \Gn{\Gamma \Sequent \Delta}$，那么 $(s)_0 = \Gn{\Gamma}$ 且 $(s)_1 = \Gn{\Delta}$。于是，
...
\end{align*}
这几行分别表达的是：“存在哥德尔数为 $g$ 的序列 ($\Gamma$)，哥德尔数为 $d$ 的序列 ($\Delta$)，哥德尔数为 $a$ 的公式 ($!A$)，以及哥德尔数为 $b$ 的公式 ($!B$)”，使得“$\pi$ 的末矢列是 $\Gamma \Sequent \Delta, !A \land !B$”“$\pi_1$ 的末矢列是 $\Gamma \Sequent \Delta, !A$”“$\pi_2$ 的末矢列是 $\Gamma \Sequent \Delta, !B$”，以及“$\pi$ 有两个直接子推导且最后的推理规则是 $\RightR\land$（编号为 $10$）”。
...
\begin{prob}
  ...
  \end{enumerate}
  对于最后一个，你还必须证明：可以原始递归地判定哥德尔数为 $p$ 的推导其最后一个推理是否满足本元变元条件，即 $\RightR{\lforall}$ 中的本元变元 $a$ 不在末矢列中出现。
  \end{prob}
  

\begin{prop}
  \ollabel{prop:deriv}
  关系 $\fn{Deriv}(p)$（当 $p$ 是正确推导 $\pi$ 的哥德尔数时成立）是原始递归的。
\end{prop}
...
\begin{prop}
假设 $\Gamma$ 是一个原始递归的语句集。那么表达“$x$ 是某个有限集 $\Gamma_0 \subseteq \Gamma$ 的推导 $\pi$ 的编码，该推导的末矢列为 $\Gamma_0 \Sequent !A$，且 $y$ 是 $!A$ 的哥德尔数”的关系 $\Prf[\Gamma](x, y)$ 是原始递归的。
\end{prop}

\begin{proof}
设“$y \in \Gamma$”由原始递归谓词~$R_\Gamma(y)$ 给出。
我们需要证明 $\Prf[\Gamma](x, y)$ 是原始递归的，其中该谓词成立当且仅当
$y$ 是语句~$!A$ 的哥德尔数，并且 $x$ 是一个 $\Log{LK}$-推导的编码，

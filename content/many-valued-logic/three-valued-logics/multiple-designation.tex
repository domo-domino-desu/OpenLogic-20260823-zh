...
\begin{enumerate}
  ...
  \item 真值集合为 $V = \{\True, \Undef, \False\}$。
  \item $\True$ 与 $\Undef$ 是指定值，即 $V^+ = \{\True, \Undef\}$。
  \item 真值函数与弱 克林 逻辑相同，但额外增加一个“无意义”算子：
  \begin{center}
    \begin{tabular}{c|c}
    $\tf{+}$ & \\
    \hline
...
与它们共用相同真值表的 克林 逻辑不同，这些逻辑\emph{确实}有重言式。
...
\begin{prob}
  通过证明以下结论来证明 \olref[mvl][thr][mul]{prop:gl-udes}：对于按 哥德尔 逻辑方式定义但指定值为 $V^+=\{\True,\Undef\}$ 的逻辑~$\Log L$，若 $\Gamma \Entails/[\Log L] !B$ 则 $\Gamma \Entails/[\LogCL] !B$。

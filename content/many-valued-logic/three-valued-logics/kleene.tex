...
克林引入了两种三值逻辑，其动机源于这样一种逻辑：其中真值被视为计算过程的结果。一个过程可能输出 $\True$ 或 $\False$，但也可能无法终止。在后一种情况下，对应的真值是未定义的，用真值~$\Undef$ 表示。
...
这两种解释分别导出了 克林 的强逻辑与弱逻辑。条件句被定义为与 $\lnot !A \lor !B$ 等价。

\begin{defn}
\emph{强 克林 逻辑}~$\LogKs$ 由以下矩阵定义：
\begin{enumerate}
...
虽然弱和强 克林 逻辑都没有重言式，但它们具有非平凡的后承关系。

\begin{prob}
  下列后承关系在（a）强和（b）弱 克林 逻辑中哪些成立？对每个关系给出真值表。
  \begin{enumerate}
...
Dmitry Bochvar（德米特里·博赫瓦尔）将 $\Undef$ 解释为“无意义”，并试图通过规定悖论性语句（如说谎者悖论）取真值~$\Undef$ 来解决此类悖论。他引入了一种逻辑，其本质上是弱 克林 逻辑增加了额外联结词的扩展，其中两个是“外部否定”和“是未定义的”算子：
\begin{center}
  \begin{tabular}{c|c} 
    $\tf{\sim}$ & \\ 
    \hline  

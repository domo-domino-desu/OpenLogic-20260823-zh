...
你会注意到，$\land$ 和~$\lor$ 的真值表与 \L ukasiewicz 逻辑和强 Kleene 逻辑中的相同，但 $\lnot$ 和~$\lif$ 的真值表在每种逻辑中各不相同。在哥德尔逻辑中，$\tf{\lnot}(\Undef) = \False$。与 \L ukasiewicz 逻辑和 Kleene 逻辑不同，$\tf{\lif}(\Undef, \False) = \False$；与 Kleene 逻辑不同（但与 \L ukasiewicz 逻辑相同），$\tf{\lif}(\Undef,
\Undef) = \True$。

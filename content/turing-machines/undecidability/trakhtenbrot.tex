...
\begin{explain}
  在 \olref[rep]{sec} 中，我们为Turing机~$M$ 和输入字符串~$w$ 定义了语句 $!T(M,w)$ 和~$!E(M,w)$。
  ...
  不幸的是，其逆命题并不成立，即存在某些Turing机 $M$ 和输入~$w$，使得 $M$ 在~$w$ 上不停机，但 $!T(M,w)
  \land !E(M,w)$ 仍然有有限模型。
  ...
  \begin{prob}
    设 $M$ 是一台只有状态 $q_0$ 和单条指令 $\delta(q_0,\TMblank) = \tuple{q,\TMblank,\TMstay}$ 的Turing机。
...
考虑描述Turing机~$M$ 在输入 $w = \sigma_{i_1}\dots\sigma_{i_k}$ 上操作的语句：
\begin{enumerate}
...
\begin{proof}
  假设存在一台Turing机~$F$ 可以判定有限可满足性问题。那么给定任意Turing机~$M$ 和输入~$w$，我们都可以计算出语句~$!T'(M,w) \land !E(M,w)$，并用~$F$ 来判定它是否有一个有限模型。根据 \cref{tur:und:tra:lem:halts-sat,tur:und:tra:lem:sat-halts}，它有有限模型当且仅当 $M$ 以输入~$w$ 启动后停机。这样我们就可以用 $F$ 来解决停机问题，但我们知道停机问题是不可解的。
\end{proof}

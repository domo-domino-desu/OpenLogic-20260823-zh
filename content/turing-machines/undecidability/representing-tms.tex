...
\begin{explain}
为了用一阶逻辑的语句来表示图灵机及其行为，我们必须定义一个合适的语言。
...
给定一台图灵机 $M = \tuple{Q, \Sigma, q_0, \delta}$，其语言~\(\Lang L_M\) 由以下符号组成：
\begin{enumerate}
...
描述图灵机~\(M\) 在输入 $w = \sigma_{i_1}\dots\sigma_{i_k}$ 上运行的语句如下：
\begin{enumerate}
\item 描述数和~$<$ 的公理：
\begin{enumerate}
\item 表示每个数都小于其后继的语句：
...
如果我们使用带有指定停机状态~$h$ 的图灵机，事情就更简单了：此时语句~$!E(M, w)$
\[

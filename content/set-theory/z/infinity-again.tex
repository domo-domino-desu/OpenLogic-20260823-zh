...
然而，这使得我们拥有\emph{无穷多个}集合，但并不能保证存在一个\emph{无穷集合}，即一个拥有无穷多成员的集合。而这确实很重要：除非我们能找到一个无穷集合，否则我们就无法构造一个Dedekind代数。但我们需要一个Dedekind代数，以便将其视为自然数集。（比较\olref[sfr][infinite][dedekindsproof]{sec}。）
...
容易看出，无穷公理提供的集合 $I$ 是Dedekind无穷的。它的特异元素是 $\emptyset$，其上的单射由 $s(x) = x\cup
\{x\}$ 给出。现在，\olref[sfr][infinite][dedekind]{thm:DedekindInfiniteAlgebra} 展示了如何从一个Dedekind无穷集合中提取出一个Dedekind代数；我们将把它视为我们的自然数集。更精确地说：
...
\begin{proof}
证明采用归纳法，即\olref[sfr][infinite][induction]{thm:dedinfiniteinduction}。显然 $0=\emptyset$ 不是Dedekind无穷的。对于归纳步骤，我们将证明其逆否命题：如果（荒谬地）$s(n)$ 是Dedekind无穷的，那么 $n$ 也是Dedekind无穷的。

因此，假设 $s(n)$ 是Dedekind无穷的，即存在某个单射 $f$ 满足 $\ran{f}\subsetneq \dom{f} = s(n) = n \cup
\{n\}$。需要考虑两种情况。

\emph{情况 1：$n \notin \ran{f}$。} 所以 $\ran{f} \subseteq n$，且 $f(n) \in n$。令 $g = \funrestrictionto{f}{n}$；则 $\ran{g} =
\ran{f} \setminus \{f(n)\} \subsetneq n = \dom{g}$。因此 $n$ 是Dedekind无穷的。
...
\]
所以，除了 $h(f^{-1}(n)) = m
\neq n = f(f^{-1}(n))$ 之外，$h$ 和 $f$ 处处一致。由于 $f$ 是单射，$n \notin \ran{h}$；并且 $\ran{h} \subsetneq \dom{h} = s(n)$。现在利用情况 1 的论证，可知 $n$ 是Dedekind无穷的。
\end{proof}
...
然而，这引出了一个明显的问题。
在 \olref[sfr][infinite][dedekindsproof]{sec} 中，我们考察了Dedekind关于存在一个（由思想构成的）Dedekind无穷集合的“证明”。
这可能并未让你觉得十分令人满意。
但如果 \stagesinf 并非由集合的迭代观念（或“思维法则”）“强加给我们的”，那么对于存在Dedekind无穷集合的主张，我们仍然缺乏内在的辩护。

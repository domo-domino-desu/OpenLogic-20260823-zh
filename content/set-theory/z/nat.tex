...
\begin{defish}
\emph{Zermelo（策梅洛）\citeyear{Zermelo1908Untersuchungen} 的无穷公理。} 存在一个集合 $A$，使得 $\emptyset \in A$ 且 $(\forall x \in A)\{x\} \in A$。
\end{defish}

假若我们采用的是 Zermelo 的公理，而非我们（受 von Neumann 启发）的无穷公理，我们同样会得到一个 Dedekind（戴德金）无限集，进而得到一个戴德金代数。在 Zermelo 的方法中，该代数的特异元同样会是 $\emptyset$（我们用以代表 $0$ 的替代物），但单射将由映射 $x \mapsto \{x\}$ 给出，而非 $x \mapsto x \cup \{x\}$。这样做最直接的结果是，Zermelo 将 $2$ 视为 $\{\{\emptyset\}\}$，而我们（与 von Neumann 一道）将 $2$ 视为 $\{\emptyset, \{\emptyset\}\}$。

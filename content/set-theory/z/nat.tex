...
我们的无穷公理沿用了 \citet{VonNeumann1925}（冯·诺伊曼）的思路。但这里还有另一条公理，我们本可以转而采用它：

\begin{defish}
\emph{策梅洛\citeyear{Zermelo1908Untersuchungen} 的无穷公理。} 存在一个集合 $A$，使得 $\emptyset \in A$ 且 $(\forall x \in A)\{x\} \in A$。
\end{defish}

假若我们采用的是 策梅洛 的公理，而非我们（受 冯·诺伊曼 启发）的无穷公理，我们同样会得到一个 戴德金无限集，进而得到一个戴德金代数。在 策梅洛 的方法中，该代数的特异元同样会是 $\emptyset$（我们用以代表 $0$ 的替代物），但单射将由映射 $x \mapsto \{x\}$ 给出，而非 $x \mapsto x \cup \{x\}$。这样做最直接的结果是，策梅洛 将 $2$ 视为 $\{\{\emptyset\}\}$，而我们（与 冯·诺伊曼 一道）将 $2$ 视为 $\{\emptyset, \{\emptyset\}\}$。

为何选择这条无穷公理而非另一条？主要的实际考量在于，冯·诺伊曼 的方法能很好地“向上扩展”以处理超限数。我们将从 \olref[ordinals][]{chap} 章开始探讨这一点。然而，仅从\emph{做算术}的简单视角来看，两种方法同样有效。所以，如果有人告诉你自然数\emph{就是}集合，那么一个明显的问题是：\emph{它们究竟是哪些集合？}

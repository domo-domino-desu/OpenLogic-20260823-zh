...
一种朴素的想法是试图这样定义 $A$ 的基数：\[
	\Setabs{x}{\cardeq{A}{x}}.
\]
你也许想把它与 弗雷格对 $\# x Fx$ 的定义作个比较，这在\olref[cardinals][hp]{sec}末尾曾有概述。而且，由于我们在那里提到的原因，这个定义行不通。任何单元素集都与 $\{\emptyset\}$ 等势。但新的单元素集在层级的每个后继阶段都会产生（只需考虑前一阶段的单元素集即可）。因此，$\Setabs{x}{\cardeq{A}{x}}$不存在，因为它不可能具有秩。

为了解决这个问题，我们使用一个由 塔斯基和 Scott（斯科特）提出的技巧：\footnote{提醒：所有公式都可能含有参数（除非另有明确说明）。}
...
在论证了 塔斯基--Scott 技巧的合理性之后，我们现在可以用它来定义基数概念：
...
顺便一提，我们也可以使用 塔斯基--Scott 技巧来发展序数理论。对于良序 $\tuple{A, <}$，令 $\text{tso}(A, <) = [\tuple{X,

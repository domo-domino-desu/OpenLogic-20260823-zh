...
序数是自然数的一种推广。它们被用来“衡量”良序集的“长度”；两个良序集具有相同的序型，当且仅当它们是同构的。% Part: set-theory我们已看到定义自然数的两种方式：Frege的方式与 von Neumann（冯·诺依曼）的方式。类似地，定义序数也有不同的方法。在本章中，我们采用一种源于 von Neumann 的方法来定义它们：序数是由传递集组成的传递集，且序关系由属于关系 $\in$ 给出。% Chapter: ordinals% Section: introduction

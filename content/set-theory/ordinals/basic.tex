...
\begin{thm}[Burali-Forti悖论]\ollabel{buraliforti}
不存在由所有序数构成的集合
\end{thm}
...
\begin{quote}
  Burali-Forti本人认为这个矛盾通过\emph{归谬法}确立了以下结果：序数的自然序只是一个偏序。
  \citep[p.~105]{Heijenoort1967}
\end{quote}
撇开Burali-Forti的错误不谈，我们可以将上述内容总结如下。序数是这样的集合：它们各自被属于关系良序，并且所有序数整体上也被属于关系良序（尽管它们整体并不构成一个集合）。

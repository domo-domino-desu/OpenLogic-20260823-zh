...
\begin{proof}
由Cantor 定理（\olref[sfr][siz][car]{thm:cantor}）与 \olref{lem:SizePowerset2Exp} 即得。
\end{proof}
...
\begin{proof}[证明概要]
证明方法有很多。最直接的方法是论证 $\cardle{\Pow{\omega}}{\Real}$ 且 $\cardle{\Real}{\Pow{\omega}}$，然后利用Schröder--Bernstein定理推断 $\cardeq{\Real}{\Pow{\omega}}$，并借助 \olref[card-arithmetic][opps]{lem:SizePowerset2Exp} 推断 $\card{\Real} = \cardexpo{2}{\omega}$。

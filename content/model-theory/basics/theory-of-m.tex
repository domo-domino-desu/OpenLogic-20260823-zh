...
\begin{rem}\ollabel{remark:R}
  考虑结构 $\Struct{R} = \langle\Real, <\rangle$，其论域为实数集 $\Real$，其语言仅包含一个被解释为实数上 $<$ 关系的二元谓词符号。显然，$\Struct{R}$ 是不可数的；然而，由于 $\Theory{R}$ 显然是一致的，根据 勒文海姆--斯科伦 定理，它有一个可数模型，记作 $\Struct{S}$；并且根据 \olref{prop:equiv}，有 $\Struct{R}

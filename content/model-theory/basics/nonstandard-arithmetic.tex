...
\Theory{N} \cup \Setabs{\num{n} < c}{n \in \Nat}.
\]
该理论是有限可满足的，因此由紧致性定理可知它有一个模型 $\Struct{M}$。根据 勒文海姆–斯科伦 下降定理，可以取 $\Struct{M}$ 为可数的。设 $\Domain{M}$ 为 $\Struct{M}$ 的论域，并设 $\Struct{M}$ 将 $\Lang{L}$ 的非逻辑常项解释为 $\mathbf{z} = \Assign{\Obj{0}}{M} \in \Domain{M}$、${\prec} =
...
\begin{prob}
集合 $X$ 上的一个关系 $R$ 是\emph{良基的}，当且仅当 $R$ 中不存在无限下降链，即不存在 $X$ 中的元素 $x_0$, $x_1$, $x_2$, \dots 使得 $\dots x_2Rx_1Rx_0$。假设 策梅洛–弗兰克尔 集合论 $ZF$ 是一致的，证明存在 $ZF$ 的\emph{非良基模型}，即满足 $\dots x_2 \in x_1 \in x_0$ 的模型 $\mathfrak{M}$。
\end{prob}

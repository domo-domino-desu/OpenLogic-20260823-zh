...
最后，通过对公式归纳可以证明，$\Struct{M}$ 在所有 $\Lang{L'_1}$ 的公式上与 $\Struct{M'_1}$ 相符，在所有 $\Lang{L'_2}$ 的公式上与 $\Struct{M'_2}$ 相符。特别地，$\Struct{M} \Entails \Gamma^* \cup \Delta^*$，因此 $\Struct{M} \Entails !A$ 且 $\Struct{M} \Entails \lnot!B$，从而 $\Entails/ !A \lif !B$。这就完成了 Craig 内插定理的证明。
\end{proof}

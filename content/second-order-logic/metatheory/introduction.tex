...
这意味着二阶逻辑不可能有可靠且完备的证明系统（尽管可靠但不完备的证明系统是存在的，并且事实上是重要的研究对象）。

一阶逻辑还有另外两个性质：它是紧致的（如果一个语句集~$\Gamma$ 的每个有穷子集都是可满足的，那么~$\Gamma$ 本身也是可满足的），
并且 Löwenheim--Skolem 定理对它成立（如果 $\Gamma$ 有一个无穷模型，那么它也有一个可数无穷模型）。
这两个结果对于二阶逻辑都不成立。原因同样在于，二阶逻辑能够表达关于论域大小的事实，而一阶逻辑则不能。
